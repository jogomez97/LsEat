...
void saveTheDishes() {
    ...
    while (1) {
        picard = checkFirstElement(&clients);
        if (picard.fd < 0) {
            //Llista buida
            break;
        }

        asprintf(&aux, "%d", picard.nPlats);
        writeTrama(picard.fd, 0x08, ENT_KO, aux);
        free(aux);
        aux = NULL;
        for (i = 0; i < picard.nPlats; i++) {
            if (picard.plats[i].quants > 0) {
                asprintf(&aux, "%s&%d", picard.plats[i].nom, picard.plats[i].quants);
                writeTrama(picard.fd, 0x04, DISH_KO, aux);
                free(aux);
                aux = NULL;
            }
        }
        deleteFirstNode(&clients);      //Internament tanca el fd
    }
